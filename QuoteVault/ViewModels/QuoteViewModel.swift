...
class QuoteViewModel: ObservableObject {
    ...
    @Published var selectedCategory: String? = nil { didSet { applyFilters() } }
    ...
    func toggleFavorite(quote: QuoteModel, userId: UUID) {
        ...
        applyFilters()
        Task { await saveFavoriteToSupabase(quote: quotes[index], userId: userId) }
    }

    func isFavorite(quote: QuoteModel) -> Bool {
        quotes.first(where: { $0.id == quote.id })?.isFavorite ?? false
    }

    private func saveFavoriteToSupabase(quote: QuoteModel, userId: UUID) async {
        do {
            let response = try await SupabaseService.client
                .from("favorites")
                .select()
            ...
            if existingFavs.isEmpty {
                try await SupabaseService.client
                    .from("favorites")
                    .insert(["user_id": userId.uuidString, "quote_id": quote.id.uuidString])
                    .execute()
            } else {
                try await SupabaseService.client
                    .from("favorites")
